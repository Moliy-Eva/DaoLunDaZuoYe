...
const devicesId = "562109393" // 填写在OneNet上获得的devicesId 形式就是一串数字 例子:9939133
const api_key = "da7mS7OtEo1xZZPMbFkg08JMkIo=" // 填写在OneNet上的 api-key 例子: VeFI0HZ44Qn5dZO14AuLbWSlSlI=
...
Page({
  ...
  getDatapoints: function () {
    return new Promise((resolve, reject) => {
      wx.request({
        url: `https://api.heclouds.com/devices/${devicesId}/datapoints?datastream_id=Temperature,Children,Humidity&limit=20`,
        /**
         * 添加HTTP报文的请求头, 
        ...
        success: (res) => {
          ...
          resolve({
            ...
            children: response.data.datastreams[2].datapoints.reverse(),
          })
        },
        fail: (err) => {
          reject(err)
  ...
  update: function (datapoints) {
    ...
    this.lineChart_children.updateData({
      categories: wheatherData.categories,
      series: [{
        name: 'children',
        data: wheatherData.children,
        format: (val, name) => val.toFixed(2)
      }],
    })
  ...
  convert: function (datapoints) {
    ...
    for (var i = 0; i < length; i++) {
      ...
      children.push(datapoints.children[i].value);
      tempe.push(datapoints.temperature[i].value); 
      console.log(datapoints.children[i].value)
    }
    return {
